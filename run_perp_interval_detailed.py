#!/usr/bin/env python3
"""
Perp Synthetic Straddle 0DTE — Detailed Interval Sweep
=======================================================
Saves individual folders with trade log, metrics, yearly CSV, and plots.

Filtered config: 25x/50x, 8K, 60%/80%, flat/compound, noTP, 0 fee, Deribit.

Usage:
  python run_perp_interval_detailed.py --interval 30 --day-filter weekday
  python run_perp_interval_detailed.py --interval 60 --day-filter weekend
"""
...
def run_sweep(timings, day_filter, df, interval_label):
    ...
    for i, ((tag, entry_t, close_t), lev, cap, alloc, (sz_label, is_flat)) in enumerate(combos, 1):
        ...
        folder_name = f"perp_{interval_label}_{day_filter}_{tag}_{lev_label}_{cap_label}_{alloc_int}pct_{sz_label}_notp"
        ...
        bt.save_summary(overall, yearly, config_dir, FEE, float(lev))
        bt.generate_plots(log, config_dir, float(lev))
...
def main():
    ...
    print(f"=== Perp Detailed Sweep: {int_label}, {day_filter} ===", flush=True)
    ...
    out_csv = OUT_DIR / f"perp_{int_label}_{day_filter}_detailed_results.csv"
    if results:
        keys = results[0].keys()
