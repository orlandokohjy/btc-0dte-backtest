#!/usr/bin/env python3
"""
ETH Pure Straddle 0DTE — Detailed Interval Sweep
==================================================
Strategy: Long 1 ATM Call + Long 1 ATM Put (options only, no spot/perp)
Adapted from BTC pure straddle script for ETH data (monthly parquets).

Usage:
  python run_eth_pure_straddle_interval_detailed.py --interval 30 --day-filter weekday
  python run_eth_pure_straddle_interval_detailed.py --interval 60 --day-filter weekend --start 0900
  python run_eth_pure_straddle_interval_detailed.py --interval 120 --day-filter weekday --capital 10000 --alloc 50
"""
...
ETH_DATA_DIR = Path(os.environ.get("ETH_DATA_DIR", "/Users/jiayikoh/Downloads/ETH_data"))
...
def load_eth_data(exchange_filter=None):
    """Load all monthly ETH parquet files."""
    files = sorted(glob.glob(str(ETH_DATA_DIR / "eth_0dte_*.parquet")))
    if not files:
        raise FileNotFoundError(f"No ETH parquet files found in {ETH_DATA_DIR}")

    print(f"Loading {len(files)} ETH parquet files ...", flush=True)
    dfs = []
    for f in files:
    ...
    # Premium conversion: Deribit premium is in ETH, needs * spot_price for USD
    # Bybit/Binance premium is already in USD
...
def save_metrics(overall, yearly, out_dir, label, initial_capital):
    lines = [
        "=" * 65,
        f"  ETH Pure Straddle 0DTE — {label}",
        "=" * 65,
...
def generate_plots(log, out_dir, initial_capital):
    ...
    ax.set_title("ETH Equity Curve (USD)"); ax.set_xlabel("Date"); ax.set_ylabel("Equity ($)"); ax.grid(True, alpha=0.3)
    ...
    ax.set_title("ETH Daily PnL (USD)"); ax.set_xlabel("Date"); ax.set_ylabel("PnL ($)"); ax.grid(True, alpha=0.3)
    ...
    ax.set_title("ETH Drawdown (%)"); ax.set_xlabel("Date"); ax.set_ylabel("DD (%)"); ax.grid(True, alpha=0.3)
    ...
    ax.set_title("ETH Monthly Returns (%)")
    ...
    ax.legend(); ax.set_title("ETH Daily PnL Distribution")
...
def run_sweep(timings, day_filter, df, interval_label, capitals=None, allocs=None):
    ...
    for i, ((tag, entry_t, close_t), cap, alloc, (sz_label, is_flat)) in enumerate(combos, 1):
        ...
        folder_name = f"eth_pure_straddle_{interval_label}_{day_filter}_{tag}_{cap_label}_{alloc_int}pct_{sz_label}_notp"
...
def main():
    ...
    print(f"=== ETH Pure Straddle Detailed Sweep: {int_label}, {day_filter} ===", flush=True)
    ...
    print("Loading ETH data (deribit only, calls+puts) ...", flush=True)
    df = load_eth_data(exchange_filter=EXCHANGE)
    print("Data loaded.\n", flush=True)

    results = run_sweep(timings, day_filter, df, int_label, capitals=capitals, allocs=allocs)

    out_csv = OUT_DIR / f"eth_pure_straddle_{int_label}_{day_filter}_{cap_tag}_{alloc_tag}_detailed_results.csv"
    if results:
        keys = results[0].keys()
