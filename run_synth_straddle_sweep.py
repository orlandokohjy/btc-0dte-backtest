#!/usr/bin/env python3
"""
Synthetic Straddle 0DTE Sweep — Deribit Only
=============================================
...
Usage:
  python run_synth_straddle_sweep.py                          # run ALL timings
  python run_synth_straddle_sweep.py --timing 1400-1800       # run ONE timing
  python run_synth_straddle_sweep.py --timing 1400-1800 --timing 1000-1400  # run selected
"""
...
def run_sweep(timings_to_run, df):
    ...
    for i, ((tag, entry_t, close_t), lev, cap, alloc, (sz_label, is_flat)) in enumerate(combos, 1):
        ...
        folder = f"0dte_synth_{tag}_{lev_label}_{cap_label}_{alloc_pct_int}pct_{sz_label}_notp_output"
...
def main():
    ...
    out_csv = OUT_DIR / f"synth_straddle_sweep_{tag_suffix}_results.csv"
    if results:
        keys = results[0].keys()
