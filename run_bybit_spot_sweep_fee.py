...
print("\nLoading data once (deribit only) ...", flush=True)
df = bt.load_data(exchange_filter="deribit")
print("Data loaded.\n", flush=True)
...
for i, (alloc, (sizing_label, is_flat), (tp_tag, tp_on)) in enumerate(combos, 1):
    alloc_label = f"{int(alloc*100)}pct"
    tp_file_tag = "" if tp_on else "_notp"

    folder = f"0dte_bybit_s3_10x_{alloc_label}_{sizing_label}{tp_file_tag}_0.5bp_output"
    ...
    if log.empty:
        ...
        results.append({
            ...
            "total_opt_contracts": 0, "total_spot_btc_vol": 0,
        })
        continue
    ...
    monthly_vol = log_copy.groupby(["year", "month"]).agg(
        trading_days=("num_straddles", "size"),
        straddles=("num_straddles", "sum"),
        opt_contracts=("option_contracts_traded", "sum"),
        opt_btc_vol=("option_btc_notional", "sum"),
        spot_btc_vol=("spot_btc_traded", "sum"),
        ...
        total_usd_vol=("usd_notional", "sum"),
    ).reset_index()
    ...
    vol_lines = [
        ...
        f"  Opt BTC per side:  {opt_btc} BTC ({bt.NUM_PUTS} puts × {bt.QTY})",
        f"  Spot BTC per side: {spot_btc} BTC",
        f"  USD volume = BTC qty × spot price, computed at entry and exit separately",
        "",
        f"{'Month':<10} {'Days':>6} {'Straddles':>10} {'OptContr':>9} {'OptBTC':>8} {'SpotBTC':>8} {'Opt$Entry':>13} {'Opt$Exit':>13} {'Spot$Entry':>13} {'Spot$Exit':>13} {'TotalUSD':>14}",
        "-" * 140,
    ]
    for _, row in monthly_vol.iterrows():
        m_str = f"{int(row['year'])}-{int(row['month']):02d}"
        vol_lines.append(
            f"{m_str:<10} {int(row['trading_days']):>6} {int(row['straddles']):>10} "
            f"{int(row['opt_contracts']):>9} {row['opt_btc_vol']:>8.1f} {row['spot_btc_vol']:>8.1f} "
            f"${row['opt_usd_entry']:>11,.0f} ${row['opt_usd_exit']:>11,.0f} "
            f"${row['spot_usd_entry']:>11,.0f} ${row['spot_usd_exit']:>11,.0f} "
            f"${row['total_usd_vol']:>12,.0f}"
        )
    vol_lines.append("-" * 140)
    vol_lines.append(
        f"{'TOTAL':<10} {monthly_vol['trading_days'].sum():>6} "
        f"{monthly_vol['straddles'].sum():>10} "
        f"{int(monthly_vol['opt_contracts'].sum()):>9} "
        f"{monthly_vol['opt_btc_vol'].sum():>8.1f} "
        f"{monthly_vol['spot_btc_vol'].sum():>8.1f} "
        ...
        f"${monthly_vol['total_usd_vol'].sum():>12,.0f}"
    )
    vol_text = "\n".join(vol_lines)
    ...
    results.append({
        ...
        "total_opt_contracts": round(monthly_vol["opt_contracts"].sum(), 1),
        "total_spot_btc_vol": round(monthly_vol["spot_btc_vol"].sum(), 1),
    })
...
# Save summary
out_csv = _SCRIPT_DIR / "output" / "bybit_spot_sweep_0.5bp_results.csv"
keys = results[0].keys()
...
print("\n" + "=" * 110)
print("BYBIT SPOT 10x SWEEP — S3 SESSION — 0.5 BTC + 2 PUTS — $8K START — NET 0.5bp FEE")
print("=" * 110)
...
print("-" * 115)
for r in results:
    opt_c = r.get("total_opt_contracts", 0)
    spot_v = r.get("total_spot_btc_vol", 0)
